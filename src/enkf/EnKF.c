...
void ReadFcst (enkf_struct ens, obs_struct obs, double *xf)
{
    ...
    for (i = 0; i < ne; i++)
    {
        ...
        xf[ne] += xf[i];
//	printf ("VAR_IND %d, weight[0] %lf, dim %d, prediction %lf\n", var_ind, obs.weight[0], ens->var[var_ind].dim, ens->member[i].var[var_ind][0]);//
    }
...
void ReadVar (char *project, char *outputdir, enkf_struct ens, int obs_time)
{
    ...
    for (i = 0; i < ne; i++)
    {
		...
		//if (k == 37) printf ("Prediction of first grid %lf\n", ens->member[i].var[k][0]);//
            }
        }
    }
